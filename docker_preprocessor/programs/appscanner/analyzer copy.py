...
x_train = pd.read_csv('./output/31_output11.csv', index_col=None)
...
drop_columns = ['Unnamed: 0', '0', '1', '2','4']

x_train = x_train.drop(drop_columns, axis=1)

x_train = pd.merge(x_train, y_train1, left_index=True, right_index=True)

x_train_unique_pcaps_labelid = x_train['Labelid'].unique()
...
def ASN_UNIQUE_FEATURES():
    for labelid in x_train_unique_pcaps_labelid:
        indexer = x_train.loc[x_train['Labelid'] == labelid].index
        ASN_UNIQUE = x_train.loc[x_train['Labelid'] == labelid][['ASN']].nunique()
        ASN_NAME_UNIQUE = x_train.loc[x_train['Labelid'] == labelid][['ASN_Name']].nunique()
...
def TCP_UDP_FEATURES():
    for labelid in x_train_unique_pcaps_labelid:
        print(labelid)
        indexer = x_train.loc[x_train['Labelid'] == labelid].index
        # indexer_tcp = x_train.loc[x_train['Labelid']]['5'] == 6 
        # indexer_udp = x_train.loc[x_train['Labelid'] == labelid]['5'] == 17
        tcp_data = x_train.loc[x_train['Labelid'] == labelid][x_train['5'] == 6]
        udp_data = x_train.loc[x_train['Labelid'] == labelid][x_train['5'] == 17]

        tcp_data_unique = tcp_data['Dst IP'].unique()
        udp_data_unique = udp_data['Dst IP'].unique()
...
print(x_train['5'].nunique())
x_train = pd.get_dummies(x_train, columns=['5'])
print(x_train.shape)
